...
public final class BinSea {
    ...
    public static int search(int[] sortedData, int value, Result result) {
        ...
        if(sortedData.length % 2 == 0){
            middleIndex = (sortedData.length/2) - 1;
        } else{
            middleIndex = (sortedData.length - 1)/2;
        }

        if(sortedData[middleIndex] == value){
            result.addStep(middleIndex);
            return middleIndex;
        }

        if(sortedData.length > 1) {
            if (sortedData[middleIndex] < value) {
                int[] sortedDataNew = new int[sortedData.length - middleIndex - 1];
                for (int i = 0; i + middleIndex + 1 < sortedData.length; i++) {
                    sortedDataNew[i] = sortedData[middleIndex + 1 + i];
                }
                result.addStep(middleIndex);
                return search(sortedDataNew, value, result);
            }

            if (sortedData[middleIndex] > value) {
                int[] sortedDataNew = new int[middleIndex];
                for (int i = 0; i < middleIndex; i++) {
                    sortedDataNew[i] = sortedData[i];
                }
                result.addStep(middleIndex);
                return search(sortedDataNew, value, result);
            }
